#Estrai DDT Fornitori BERARDI SENZA FTP
...
class t:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("ESTRAI DDTF BERARDI (no FTP)")     # Add a title
    ...
    def caricaDDTC(self):
        ...
        row_c = cursor.execute("SELECT  COUNT(*) FROM GAMMAtest.[dbo].[1_Archiva_DDTVSFOR] where isnull(NomeFilePerArchiva,'') <>'' AND DataDocumento >= ? AND DataDocumento <= ? and NumeroDocumentaleInterno not in (select idElab from _APPO_DDTVSFOR_archiva)", self.beginDate, self.endDate)
        ...
        while row_count!='0 ':
            ...
            pathFileTemporaneo="C:/Users/administrator.BERARDI/Desktop/MarcoTEST/DDTF_BERARDI/" + stringa3SenzaApici2
            ...
            cursor.execute("INSERT INTO [GAMMAtest].[dbo].[_APPO_DDTVSFOR_archiva] (idElab) VALUES  ("+stringa1SenzaParentesi+")")
            cnxn.commit()
    ...
    def formatRowCount(row_count_string):
        charToDeleteInrow_count="[(,)]"
        for char in charToDeleteInrow_count:
            formatted_row_count = row_count_string.replace(char,"")
        return formatted_row_count
