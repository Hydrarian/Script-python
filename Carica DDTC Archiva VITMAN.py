...
ssh_client.connect(hostname='ftp.archivagroup.it',port=2411,username='vitmansrl',password='HMFs>3cm@tez') 
...
class t:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Carica DDTC VITMAN")     # Add a title
    ...
    def contaRecord(self, cursor_):
        ...
        row_c = cursor_.execute("SELECT  COUNT(*) FROM   VITMAN.dbo.[1_Archiva_DDTC] where isnull(NomeFilePerArchiva,'') <>'' AND DataProtocollo >= ? AND DataProtocollo <= ? and IdObject not in (select idElab from _APPO_ddtc_archiva)", self.beginDate, self.endDate)
    ...
    def caricaDDTC(self):
        ...
        while row_count!='0 ':
            ...
            #Eseguo la query:
            row = cursor.execute("SELECT  top 1 [IdObject], [myPathName], [myFileName] FROM   VITMAN.dbo.[1_Archiva_DDTC] where isnull(NomeFilePerArchiva,'') <>'' AND DataProtocollo >= ? AND DataProtocollo <= ? and IdObject not in (select idElab from _APPO_ddtc_archiva) order by [NumeroProtocollo], [myDataFileName] desc", self.beginDate, self.endDate)
            ...
            try:
                ...
                cursor.execute("INSERT INTO [VITMAN].[dbo].[_APPO_ddtc_archiva] (idElab) VALUES  ("+stringa1SenzaParentesi+")")
                cnxn.commit()
