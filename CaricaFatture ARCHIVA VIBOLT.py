...
row_c = cursor.execute("SELECT  COUNT(*) FROM   VIBOLT.dbo.[1_Archiva_DDTC] where isnull(NomeFilePerArchiva,'') <>'' AND DataProtocollo >= '2019-01-01' and IdObject not in (select idElab from _APPO_ddtc_archiva)")
...
while row_count!="[(0, )]":
	#Eseguo la query:
	row = cursor.execute("SELECT  top 1 [IdObject], [myPathName], [myFileName] FROM   VIBOLT.dbo.[1_Archiva_DDTC] where isnull(NomeFilePerArchiva,'') <>'' AND DataProtocollo >= '2019-01-01' and IdObject not in (select idElab from _APPO_ddtc_archiva) order by IdObject asc")
