...
class t:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Carica DDTC VIBOLT")     # Add a title
    ...
    def contaRecord(self, cursor_):
        ...
        row_c = cursor_.execute("SELECT  COUNT(*) FROM   VIBOLT.dbo.[1_Archiva_DDTC] where isnull(NomeFilePerArchiva,'') <>'' AND DataProtocollo >= ? AND DataProtocollo <= ? and IdObject not in (select idElab from _APPO_ddtc_archiva)", self.beginDate, self.endDate)
    ...
    def caricaDDTC(self):
        ...
        while row_count!='0 ':
            ...
            #Eseguo la query:
            row = cursor.execute("SELECT  top 1 [IdObject], [myPathName], [myFileName] FROM   VIBOLT.dbo.[1_Archiva_DDTC] where isnull(NomeFilePerArchiva,'') <>'' AND DataProtocollo >= ? AND DataProtocollo <= ? and IdObject not in (select idElab from _APPO_ddtc_archiva) order by [NumeroProtocollo], [myDataFileName] desc", self.beginDate, self.endDate)
            ...
            try:
                ...
                cursor.execute("INSERT INTO [VIBOLT].[dbo].[_APPO_ddtc_archiva] (idElab) VALUES  ("+stringa1SenzaParentesi+")")
                cnxn.commit()
