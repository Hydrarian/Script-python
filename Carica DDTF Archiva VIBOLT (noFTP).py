#Estrai DDT Fornitori VIBOLT SENZA FTP
...
class t:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("ESTRAI DDTF VIBOLT (no FTP)")     # Add a title
    ...
    def caricaDDTC(self):
        ...
        row_c = cursor.execute("SELECT  COUNT(*) FROM VIBOLT.[dbo].[1_Archiva_DDTVSFOR] where isnull(NomeFilePerArchiva,'') <>'' AND DataDocumento >= ? AND DataDocumento <= ? and NumeroDocumentaleInterno not in (select idElab from _APPO_DDTVSFOR_archiva)", self.beginDate, self.endDate)
        ...
        while row_count!='0 ':
            ...
            pathFileTemporaneo="C:/Users/administrator.BERARDI/Desktop/MarcoTEST/DDTF_VIBOLT/" + stringa3SenzaApici2
