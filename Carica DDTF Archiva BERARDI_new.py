...
class t:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Carica DDTF BERARDI")     # Add a title
    ...
    def contaRecord(self, cursor_):
        ...
        row_c = cursor_.execute("SELECT  COUNT(*) FROM GAMMAtest.[dbo].[1_Archiva_DDTVSFOR] where isnull(NomeFilePerArchiva,'') <>'' AND DataDocumento >= ? AND DataDocumento <= ? and NumeroDocumentaleInterno not in (select idElab from _APPO_DDTVSFOR_archiva)", self.beginDate, self.endDate)
